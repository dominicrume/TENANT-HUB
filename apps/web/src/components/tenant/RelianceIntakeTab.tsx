      ...
      if (match && match.data && Object.keys(match.data).length > 0) {
        ...
        const initialPack: ReliancePack = {
          ...
          referral: {
            ...
            title: tenant.title,
            full_name: tenant.full_name, // Using our schema aliases
          ...
          <div style={{ background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: "12px", padding: "20px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              {[
                ...
                const isChecked = !!data.tracker[item.key as keyof ReliancePackTracker];
                return (
                  <div key={item.key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px", background: "#fff", borderRadius: "8px", border: "1px solid #E2E8F0" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                      <input 
                        type="checkbox" 
                        checked={isChecked}
                        onChange={(e) => handleTrackerChange(item.key as any, e.target.checked ? today : "")}
                    ...
                    {isChecked && <span style={{ fontSize: "12px", color: "#1E7F4F", fontWeight: 600 }}>Completed {data.tracker[item.key as keyof ReliancePackTracker]}</span>}
                  </div>
                )
              })}
            </div>
      ...
      {step === "referral" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          <div style={{ background: "#F8F4EF", padding: "20px", borderRadius: "12px", border: "1px solid #EDE8E1" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              <TextInput label="Full Name" value={data.referral.full_name} onChange={(v) => handleRefChange("full_name", v)} />
              <TextInput label="NINO" value={data.referral.nino} onChange={(v) => handleRefChange("nino", v)} />
              <TextInput label="Date of Birth" type="date" value={data.referral.dob} onChange={(v) => handleRefChange("dob", v)} />
              <TextInput label="Mobile" value={data.referral.mobile_no} onChange={(v) => handleRefChange("mobile_no", v)} />
            </div>
          </div>
          ...
          <div style={{ background: "#fff", padding: "20px", borderRadius: "12px", border: "1px solid #EDE8E1" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              <TextInput label="Registered GP" value={data.referral.registered_gp} onChange={(v) => handleRefChange("registered_gp", v)} />
              <TextInput label="Physical Health Conditions" value={data.referral.physical_health} onChange={(v) => handleRefChange("physical_health", v)} />
              <TextInput label="Mental Health Conditions" value={data.referral.mental_health} onChange={(v) => handleRefChange("mental_health", v)} />
              <TextInput label="Prescribed Medication" value={data.referral.medication} onChange={(v) => handleRefChange("medication", v)} />
      ...
      {step === "missing-person" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
           <div style={{ background: "#fff", padding: "20px", borderRadius: "12px", border: "1px solid #EDE8E1" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              <TextInput label="Height" value={data.missing_person.height} onChange={(v) => handleMissingChange("height", v)} />
              <TextInput label="Build" value={data.missing_person.build} onChange={(v) => handleMissingChange("build", v)} />
              <TextInput label="Hair Color" value={data.missing_person.hair_color} onChange={(v) => handleMissingChange("hair_color", v)} />
              <TextInput label="Eye Color" value={data.missing_person.eye_color} onChange={(v) => handleMissingChange("eye_color", v)} />
              <TextInput label="Distinguishing Marks (Tattoos/Birthmarks)" value={data.missing_person.distinguishing_marks} onChange={(v) => handleMissingChange("distinguishing_marks", v)} />
            </div>
          </div>
      ...
      {step === "consents" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          <ConsentBox 
            ...
            onChange={(sign, dt) => { handleConsentChange("bcc_housing_benefit_consent_signed_by", sign); handleConsentChange("bcc_housing_benefit_consent_date", dt); }}
            critical
          />
          <ConsentBox 
            ...
            onChange={(sign, dt) => { handleConsentChange("data_protection_signed_by", sign); handleConsentChange("data_protection_date", dt); }}
          />
          <ConsentBox 
          ...
          <ConsentBox 
            ...
            onChange={(sign, dt) => { handleConsentChange("fire_evacuation_signed_by", sign); handleConsentChange("fire_evacuation_date", dt); }}
          />
        </div>
      )}
